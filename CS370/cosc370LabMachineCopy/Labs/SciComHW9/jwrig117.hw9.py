...
#  
# Romberg integration is a combination of the trapezoidal rule and richardson extrapolation
# where in each integration approximation is used with the previous one to reduce the overall
# error of the calculation and then each successive romberg calculation is used with a previous
# one to achieve an even higher level of accuracy
# Joshua Wright HW 9
...
def f(x):
    if x == 0:
        ...
        return (x ** 4 * e ** x) / (e ** x - 1) ** 2  # define the integrable function here
...
for i in u:
    if i == 0:
        ...
        I, nPanels = romberg(f, 0.0, 1.0 / i)  # perform romberg integration on f here (get result in
...
plt.xlim(0.0, 1.0)
plt.ylim(0.00, 0.35)
plt.plot(u, gu, color='blue', linewidth=.5)
plt.savefig("prob6_1_14")
plt.show()
