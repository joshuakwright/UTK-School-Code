...
#  block also.
#
#  R = this is the trajectory aka the radius above 0,0
#  theta = this is the degree above (relatively speaking) the horizontal axis (the 0 degree axis)
#  C = this is a constant that if found using a combination of a gravitational coefficient, masses of the object, and
#  angular momentum's
#  e = this is the eccentricity of orbit aka how much the orbit is squished to look like a oval rather than a circle
#  alpha = this is the semi-major axis which defines the size of the orbit
#  Joshua Wright HW 7
#
...
def F(x):
    ...
    F[0] = 6870.0 - x[0] / (1 + x[1] * sin(-pi / 6 + x[2]))
    F[1] = 6728.0 - x[0] / (1 + x[1] * sin(x[2]))
    F[2] = 6615.0 - x[0] / (1 + x[1] * sin(pi / 6 + x[2]))
    return F


# Initial guess
x = np.array([6800.0, 0.5, 0.0])  # What list needs to be used to initialze the Newton-Raphson (N-R) method?
